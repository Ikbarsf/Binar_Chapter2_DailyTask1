...
function getRegistered(data){
    // buat variabel result berupa array kosong yang nantinya untuk menyimpan data yang sudah difilter 
    let result = []
    // looping cek semua data
    for(let i = 0; i < data.length; i++){
        // buat filter untuk user yang registed antara 2014-2018
        if(data[i].registered >= '2014' && data[i].registered <= '2018'){
            // push data ke variabel result
            result.push(data[i])
        }
    }
    // mengembalikan data
...
console.log(`jumlah user yang regisrated pada antara 2014-2018 ada ${getRegistered(users).length}`)